...
def _role_config_items_for_creation(
        policy: RbacPolicy,
        cfg: dict
        ) -> list[dict]:
    model = cfg["model"]
...
def _normalize_role_creation_links(
        raw_links
        ) -> dict[str, set[int]]:
...
def _parse_qty_integer_decimal(
        value,
        field_name: str = "Quantità"
        ) -> Decimal:
...
def _ordine_has_distinta_materiale(
        ordine
        ) -> bool:
...
def _phase_sequence_for_ordine(
        ordine
        ) -> list[str]:
...
def _order_hours_snapshot_complessiva(
        ordine: InputOdp
        ) -> tuple[float, float]:
...
def _order_hours_snapshot_reparto(
        ordine: InputOdp
        ) -> float:
    fase_attiva = _norm_text(getattr(ordine, "FaseAttiva", "")) or "1"
    ore_lavorazione = InputOdp._active_value_from_phase_list(
...
def _dash_complessiva_finalize_bucket(
        bucket: dict
        ) -> dict:
...
def _sync_active_fields_for_phase(
        ordine,
        fase_corrente: str | None = None
        ) -> None:
...
def _normalize_lotti_for_payload(
...
def _normalize_lotto_prodotto_for_payload(
        lotto: dict | None
        ) -> dict | None:
    if not lotto:
        return None
...
def _current_username(
        default: str = "utente_sconosciuto"
        ) -> str:
    return (
...
def _norm_articolo_search_value(
        value
        ) -> str:
    return str(value or "").strip().upper()


def _norm_articolo_revisione(
        value
        ) -> str:
...
def _get_pending_avp_export_rows(
        outbox_id: int | None = None
        ) -> list[dict]:
    rows = []
...
def _normalize_indice_modifica_for_pdf(
        value
        ) -> str:
...
def _scan_montaggio_pdf_directory(
        pdf_dir: Path
        ) -> dict[str, Path]:
    files = {}
...
def _build_metodo_montaggio_lookup(
        odp_rows
        ) -> dict[str, dict]:
    lookup = {}
...
def _normalize_article_search_token(
        value
        ) -> str:
    return _norm_text(value)


def _normalize_variante_articolo_search(
        value
        ) -> str:
...
def _normalize_indice_articolo_search(
        value
        ) -> str:
...
def _scan_materiale_image_directory(
        img_dir: Path
        ) -> dict[str, Path]:
    files = {}
...
def _priorita_rows_for_operatore(
        operatore_id: int
        ) -> list[OdpPriorita]:
    return (
...
def _ordini_pianificata_visibili_per_operatore(
        operatore: User
        ) -> list[InputOdp]:
    """
...
def _cleanup_priorita_operatore(
        operatore: User
        ) -> None:
    """
...
def _compact_priorita_operatore(
        operatore_id: int
        ) -> None:
    """
    Ricompatta la coda:
...
def _ordine_priorita_payload(
        ordine,
        priorita_row: OdpPriorita | None = None
        ) -> dict:
...
def _normalize_to_business_day(
        day_value: date
        ) -> date:
    current = day_value
...
def _calc_supply_date_from_today(
        lead_time_days
        ) -> date | None:
    try:
...
def _parse_registration_date_input(
        value
        ) -> date | None:
    raw = _norm_text(value)
    if not raw:
...
def _delete_closed_order_from_runtime_db(
        ordine,
        stato = None
        ) -> None:
    """
...
def _build_public_id_from_full_name(
        value: str
        ) -> str:
    raw = _norm_text(value)
    if not raw:
    ...
    if show_user_abac_section:
        ...
        for ruolo in ruoli_gestibili:
            ...
            lavorazioni = sorted(
                    ...
                    key = lambda
                        x: ((x.Codice or "").lower(), (x.Descrizione or "").lower()),
                    )
            risorse = sorted(
...
def _remaining_phase_codes_for_ordine(
        ordine
        ) -> set[str]:
    fasi = _phase_sequence_for_ordine(ordine)
...
def _build_acquisti_materiale_rows() -> list[dict]:
    ...
    for _, row in grouped.items():
        ...
        row["OrdineDettagli"].sort(key = lambda
            x: ((x.get("Ordine") or "").lower(),))
...
def _build_acquisti_excel_workbook(
        section: str,
        rows: list[dict]
        ) -> Workbook:
    wb = Workbook()
...
@login_required
@require_perm("priorita_view")
def api_priorita_ordini_operatore(
        operatore_id: int
        ):
    ...
    payload["available"].sort(key = lambda
        x: (x["ordine"], x["fase"]))
    payload["p1"].sort(key = lambda
        x: x["posizione"] or 0)
    payload["p2"].sort(key = lambda
        x: x["posizione"] or 0)
    payload["p3"].sort(key = lambda
        x: x["posizione"] or 0)
...
@login_required
@require_perm("priorita_edit")
def api_priorita_salva_operatore(
        operatore_id: int
        ):
    operatore = _get_priorita_visible_operatore_or_403(operatore_id)
    operatore_id = operatore.id
...
def _priorita_manageable_role_ids_for_user(
        user: User
        ) -> set[int]:
    """
...
def _get_priorita_visible_operatore_or_403(
        operatore_id: int
        ) -> User:
    """
...
@login_required
@require_perm("priorita_edit")
def api_priorita_reset_operatore(
        operatore_id
        ):
    operatore = _get_priorita_visible_operatore_or_403(operatore_id)
    ...
    try:
        deleted_count = (
            OdpPriorita.query
            .filter(OdpPriorita.operatore_id == operatore.id)
